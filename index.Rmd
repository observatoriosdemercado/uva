...
### Mínimo, Máximo, Media e preços de 2021 a 2026 de Uva Branca Embalada {.no-title}
...
```{r negra1, results='', fig.cap='', fig.align='center'}
...
### Principais Destinos da Uva Exportada em Fevereiro de 2026 {.no-title}
...
### Principais vias de exportação da uva do Brasil: Fevereiro de 2026. {.no-title}
...
### Volume das Exportações de Uva do Brasil: Fevereiro de 2026 em comparação com as informações entre 2016-2025 {.no-title}
...
saldo_26 <- dados1a |> filter(ano == "2026") |> select(ano, mes, Saldo_uva) |> mutate(mes = as.character(mes))
...
saldo_26 <- dados1a |> filter(ano == "2026") |> select(ano, mes, Saldo_uva)
...
total_26$mes <- factor(total_26$mes,levels = c("Janeiro", "Fevereiro", "Março", "Abril", "Maio", 
...
### pela viticultura em `r strftime(atual, format = "%B")` de 2026 
``` {r emp1}
...
### Saldo Gerado pela viticultura em `r strftime(atual, format = "%B")` de 2026
``` {r emp3}
...
g1 <- ggplot() +  #estetica vai valer para todos os geom's
  geom_col(data=saldom, aes(x=mes, y=Saldo_uva, fill=ano), size=2, width = 0.9, position = "dodge")+
    scale_fill_manual(values=mycolors2)+
    labs(y= "Saldo de Emprego (unidade)", x= "Meses do Ano", title='Saldo mensal de Empregos na Viticultura: 2025 e 2026',
       caption = "")+
