...
### Volume das Exportações de Uva do Brasil: Janeiro de 2026 em comparação com as informações entre 2016-2025 {.no-title}
...
subclasse_positivos <-dados_12_vsf_positivos%>% filter(subclasse=="132600")
subclasse_negativos <-dados_12_vsf_negativos%>% filter(subclasse=="132600")
...
g1 <- ggplot() +  #estetica vai valer para todos os geom's
  geom_col(data=saldom, aes(x=mes, y=Saldo_uva, fill=ano), size=2, width = 0.9, position = "dodge")+
    scale_fill_manual(values=mycolors2)+
    labs(y= "Saldo de Emprego (unidade)", x= "Meses do Ano", title='Saldo mensal de Empregos na Viticultura: 2023 e 2024',
       caption = "")+
...
g4 <- ggplot(data = subclasse_positivos, aes(x = grau_instrucao, y = ..count..)) +
  geom_bar(fill='#1e4971')+
    scale_y_continuous(limits=c(0, 850), n.breaks = 10, expand = expansion(add=c(0,0.5)))+
