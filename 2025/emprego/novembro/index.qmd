...
Em `r strftime(atual, format = "%B de %Y")`, o Estoque de empregos na cultura da uva é de 30.531 pessoas. No mesmo mês do ano passado, este valor era de 31.890, ou seja, ocorreu um aumento do número de pessoas ocupadas na viticultura de 1,54%, nesta comparação.

O saldo de empregos na viticultura em `r strftime(atual, format = "%B de %Y")` no Vale do São Francisco é negativo, de -1.346 pessoas, ou seja, o total de contratações (1.798) foi menor do que o total de demissões (3.144). Em relação ao mesmo mês do ano passado, se tem uma piora no saldo total (-1.257 em 2024 e -1.346 em 2025), pois o número de pessoas que perderam o emprego foi maior. 

Dos contratados no mês, 900 são do sexo masculino (50,05% do total) e 898 do sexo feminino (49,94%). Em relação à idade, 13,90% dos contratados tem até 21 anos, 12,07% até 24 anos, 12,46% até 27 anos e 9,18% até 30 anos.

Sobre o nível de escolaridade, a maioria das admissões é de pessoas com ensino médio completo (781). Do total de contratados, 43,43% possuem este nível de escolaridade.

Com relação aos salários pagos, 85,43% dos contratados foram pelo valor de R\$ 1.555,00 (valor definido no CCT 2025), mais alto que o salário mínimo nacional. O maior salário foi R$ 9.296,00. 
...
estoque_wide <- estoquem %>%
  ...
  group_by(ano, mes) %>%
  summarise(
    uva_total = if (all(is.na(uva_total))) NA_real_ else sum(uva_total, na.rm = TRUE),
    .groups = "drop"
  ) %>%
