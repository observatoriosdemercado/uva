...
Em `r strftime(atual, format = "%B de %Y")`, o Estoque de empregos na cultura da uva é de 31.890 pessoas. No mesmo mês do ano passado, este valor era de 31.332, ou seja, ocorreu um aumento do número de pessoas ocupadas na viticultura de 1,78%, nesta comparação.

O saldo de empregos na viticultura em `r strftime(atual, format = "%B de %Y")` no Vale do São Francisco é negativo, de -616 pessoas, ou seja, o total de contratações (2.190) foi menor do que o total de demissões (2.806). Em relação ao mesmo mês do ano passado, se tem um aumento no saldo total (-684 em 2024 e -616 em 2025), pois o número de pessoas que mantiveram o emprego foi maior. 

Dos contratados no mês, 1.092 são do sexo feminino (49,86% do total) e 1.098 do sexo masculino (50,14%). Em relação à idade, 13,65% dos contratados tem até 21 anos, 13,06% até 24 anos, 9,91% até 27 anos e 9,09% até 30 anos.

Sobre o nível de escolaridade, a maioria das admissões é de pessoas com ensino médio completo (965). Do total de contratados, 44,06% possuem este nível de escolaridade.

Com relação aos salários pagos, 83,38% dos contratados foram pelo valor de R\$ 1.555,00 (valor definido no CCT 2025), mais alto que o salário mínimo nacional. O maior salário foi R$ 5.903,00. 
...
#Entrando dados no R
dados1 <- read.csv2('outubro_2025.csv', header=T, sep=";")
...
#Entrando dados no R
dados2 <- read.csv2('estoque_outubro25.csv', header=T, sep=";")
