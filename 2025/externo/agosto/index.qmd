...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 847 toneladas (t). A média histórica (até 2024) para este mês é de 584 t. Em `r strftime(atual, format = "%B")` de 2023 foram exportadas 751 t e, em `r strftime(atual, format = "%B")` de 2024, foram 828 t, ou seja, houve um aumento de 12,78% em relação a 2023 e de 2,29% na comparação com o mesmo período de 2024. No ano, foram exportadas 12,4 mil t, aumento de 95,4% em relação à 2024.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de US\$ 1,69 milhões de dólares. A média histórica (até 2024) para este mês é de aproximadamente US\$ 1,20 milhÕes. Em `r strftime(atual, format = "%B")` de 2023 foram gerados US\$ 1,90 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2024, um total de US\$ 2,11 milhões. No ano, as receitas chegam a US$ 31,5 milhões, aumento de 73% em relação à 2024.

Os principais destinos foram a Holanda (29,33%), a Argentina (26,08%), o Reino Unido (24,24%) e os Estados Unidos (13,49%). A principal via de envio foi a maritima (69,56%), com 45,95% do total despachado pela alfândega de Fortaleza e 24,26% pela de Salvador.

Os principais estados de origem da uva foram Pernambuco (60,88%) e Bahia (34,35%). Em `r strftime(atual, format = "%B de %Y")`, cerca de 67 t de uvas foram importadas do Peru.
