...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 1.380 toneladas (t). A média histórica (até 2024) para este mês é de 2.168 t. Em `r strftime(atual, format = "%B")` de 2023 foram exportadas 3.694 t e, em `r strftime(atual, format = "%B")` de 2024, foram 761 t, ou seja, houve uma redução de -62,56% em relação a 2023 e um aumento de 81,73% na comparação com o mesmo período de 2024. No ano, foram exportadas 4,25 mil t, aumento de 8,3% em relação à 2024.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de US\$ 3,4 milhões de dólares. A média histórica (até 2024) para este mês é de aproximadamente US\$ 5,2 milhões. Em `r strftime(atual, format = "%B")` de 2023 foram gerados US\$ 9 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2024, um total de US\$ 2,1 milhões. No ano, as receitas chegam a US$ 7,7 milhões, redução de -8,67% em relação à 2024.

Os principais destinos foram a Holanda (41,89%), os Estados Unidos (29,8%) , o Reino Unido (21,83%) e a Espanha (2,03%). A principal via de envio foi a maritima (98,73%), com 60,64% do total despachado pela alfândega de Fortaleza e 37,07% pela de Salvador.

Os principais estados de origem da uva foram Pernambuco (64,76%) e Bahia (33,48%). Em Abril, cerca de 1,8 mil t de uvas foram importadas do Chile.
...
dados21$URF <- factor(dados21$URF, levels = c("ALF-Fortaleza", "ALF-Salvador", "Aeroporto de GRU", "ALF-Foz do Iguaçu", "Porto de Santos", "Porto do Rio de Janeiro", "Outros"))
