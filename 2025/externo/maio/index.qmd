...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 3.087 toneladas (t). A média histórica (até 2024) para este mês é de 3.274 t. Em `r strftime(atual, format = "%B")` de 2023 foram exportadas 7.902 t e, em `r strftime(atual, format = "%B")` de 2024, foram 460 t, ou seja, houve uma redução de -60,93% em relação a 2023 e um aumento de 571,09% na comparação com o mesmo período de 2024. No ano, foram exportadas 7,33 mil t, aumento de 67,36% em relação à 2024.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de US\$ 7,47 milhões de dólares. A média histórica (até 2024) para este mês é de aproximadamente US\$ 7,41 milhões. Em `r strftime(atual, format = "%B")` de 2023 foram gerados US\$ 19,9 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2024, um total de US\$ 1,38 milhões. No ano, as receitas chegam a US$ 18,3 milhões, aumento de 37,34% em relação à 2024.

Os principais destinos foram a Holanda (37,62%), o Reino Unido (27,56%), os Estados Unidos (22,55%) e a Argentina (6,59%). A principal via de envio foi a maritima (90,29%), com 57,64% do total despachado pela alfândega de Fortaleza e 34,10% pela de Salvador.

Os principais estados de origem da uva foram Pernambuco (62,59%) e Bahia (34,61%). Em `r strftime(atual, format = "%B de %Y")`, cerca de 1,2 mil t de uvas foram importadas do Chile.
...
dados21$URF <- factor(dados21$URF, levels = c("ALF-Fortaleza", "ALF-Salvador", "ALF-Foz do Iguaçu", "ALF-Dionísio Cerqueira", "Aeroporto de GRU", "IRF-Chuí", "Porto de Santos", "Porto do Rio de Janeiro", "Outros"))
