...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 2.062 toneladas (t). A média histórica (até 2024) para este mês é de 1.151 t. Em `r strftime(atual, format = "%B")` de 2023 foram exportadas 1.402 t e, em `r strftime(atual, format = "%B")` de 2024, foram 1.754 t, ou seja, houve um aumento de 47,07% em relação a 2023 e de 17,56% na comparação com o mesmo período de 2024. 

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 5,326 milhões de dólares (US\$). A média histórica (até 2024) para este mês é de aproximadamente US\$ 2,84 milhões. Em `r strftime(atual, format = "%B")` de 2023 foram gerados US\$ 3,25 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2024, um total de US\$ 5,36 milhões.

Os principais destinos foram os Estados Unidos (39,94%), a Holanda (36,84%), o Reino Unido (16,26%) e o Canadá (2,21%). A principal via de envio foi a maritima (99,61%), com 61,66% do total despachado pela alfândega de Fortaleza e 37,36% pela de Salvador.

Os principais estados de origem da uva foram Pernambuco (71,32%) e Bahia (27,72%). Em Janeiro, cerca de 389 t de uvas foram importadas do Chile, 346 t da Argentina e 36 t do Peru.
