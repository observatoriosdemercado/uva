...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 480 toneladas (t). A média histórica (até 2024) para este mês é de 1.032 t. Em `r strftime(atual, format = "%B")` de 2023 foram exportadas 889 t e, em `r strftime(atual, format = "%B")` de 2024, foram 606 t, ou seja, houve uma redução de -46% em relação a 2023 e de -20,79% na comparação com o mesmo período de 2024. No ano, foram exportadas 2,9 mil t, queda de -9,4% em relação à 2024.

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de US\$ 1,351 milhões de dólares. A média histórica (até 2024) para este mês é de aproximadamente US\$ 2,834 milhões. Em `r strftime(atual, format = "%B")` de 2023 foram gerados US\$ 2,833 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2024, um total de US\$ 2,126 milhões. No ano, as receitas chegam a US$ 7,7 milhões, redução de -10,63% em relação à 2024.

Os principais destinos foram os Estados Unidos (39,33%), a Holanda (35,76%), o Reino Unido (20,18%) e a Espanha (2,12%). A principal via de envio foi a maritima (99,3%), com 64,44% do total despachado pela alfândega de Fortaleza e 33,71% pela de Salvador.

Os principais estados de origem da uva foram Pernambuco (67,92%) e Bahia (30,39%). Em Março, cerca de 1,8 mil t de uvas foram importadas do Chile e 182 t da Argentina.
