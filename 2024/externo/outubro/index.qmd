...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 17.019 toneladas (t). A média histórica (até 2023) para este mês é de 15.408 t. Em `r strftime(atual, format = "%B")` de 2022 foram exportadas 11.473 t e, em `r strftime(atual, format = "%B")` de 2023, foram 14.983 t, ou seja, houve um aumento de 48,33% em relação a 2022, e de 13,59% na comparação com o mesmo período de 2023. A análise dos dez meses de 2024, mostra uma queda de -25,73% no volume exportado de uva pelo Brasil em relação a 2023.

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 42,4 milhões de dólares (US\$). A média histórica (até 2023) para este mês é de aproximadamente US\$ 33,8 milhões. Em `r strftime(atual, format = "%B")` de 2022 foram gerados US\$ 25,9 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2023, um total de US\$ 39,8 milhões.

Os principais destinos no ano foram a Holanda (33,3%), o Reino Unido (24,11%), os Estados Unidos (20,73%) e a Argentina (9,44%). A principal via de envio foi a maritima (88,7%), com 46,83% do total despachado pela alfândega de Salvador e 41,14% pela de Fortaleza. 

Os principais estados de origem da uva foram Pernambuco (67,7%) e Bahia (29,19%). No ano, cerca de 6,4 mil t de uvas foram importadas do Chile, 1,4 mil t da Argentina, 763 t do Peru, 64 t do Egito e 1 t de Portugal.
...
dados21$URF <- factor(dados21$URF, levels = c("ALF-Salvador", "ALF-Fortaleza", "ALF-Foz", "Aeroporto de GRU", "ALF-Corumba", "Outros"))
