...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 1.754 toneladas (t). A média histórica (até 2023) para este mês é de 1.075 t. Em `r strftime(atual, format = "%B")` de 2022 foram exportadas 1.124 t e, em `r strftime(atual, format = "%B")` de 2023, foram 1.402 t, ou seja, houve um aumento de 56,05\% em relação a 2022, e de 25,11\% na comparação com o mesmo período de 2023. 

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 4,6 milhões de dólares (US\$). A média histórica (até 2023) para este mês é de aproximadamente US\$ 2,52 milhões. Em `r strftime(atual, format = "%B")` de 2022 foram gerados US\$ 2,85 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2023, um total de US\$ 3,25 milhões.

Os principais destinos no ano foram os Estados Unidos (79,4\%), a Holanda (8\%) o Reino Unido (5,5\%) e a Espanha (2,5\%). A principal via de envio foi a maritima (99\%) e os principais estados de origem da uva foram Pernambuco (72%) e Bahia (26,6\%). No ano, cerca de 188 t de uvas foram importadas do Chile e 172 t da Argentina.
...
# 4. Principais destinos de exportação da uva do Brasil: `r strftime(atual, format = "%B")` de 2024.
...
# 5. Principais vias de exportação da uva do Brasil: `r strftime(atual, format = "%B")` de 2024. 
...
# 6. Unidade da Federação da uva exportada do Brasil: `r strftime(atual, format = "%B")` de 2024. 
