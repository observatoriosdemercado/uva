...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 3.171 toneladas (t). A média histórica (até 2023) para este mês é de 3.324 t. Em `r strftime(atual, format = "%B")` de 2022 foram exportadas 2.279 t e, em `r strftime(atual, format = "%B")` de 2023, foram 3.388 t, ou seja, houve uma queda de -6,40% em relação a 2023, mas um aumento de 39,14% na comparação com o mesmo período de 2022. A análise dos nove meses de 2024, mostra uma queda de -54,11% no volume exportado de uva pelo Brasil em relação a 2023.

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 909 mil de dólares (US\$). A média histórica (até 2023) para este mês é de aproximadamente US\$ 7,49 milhões. Em `r strftime(atual, format = "%B")` de 2022 foram gerados US\$ 5,26 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2023, um total de US\$ 10 milhões.

Os principais destinos no ano foram os Estados Unidos (37,88%), o Reino Unido (21,25%) e a Argentina (18,41%). A principal via de envio foi a maritima (77,8%), com 53,95% do total despachado pela alfândega de Fortaleza e 23,05% pela de Salvador. 

Os principais estados de origem da uva foram Pernambuco (61,4%) e Bahia (32,9%). No ano, cerca de 6,4 mil t de uvas foram importadas do Chile, 1,4 mil t da Argentina, 615 t do Peru, 64 t do Egito e 1 t de Portugal.
...
dados21$URF <- factor(dados21$URF, levels = c("ALF-Fortaleza", "ALF-Salvador", "ALF-Foz", "Aeroporto de GRU", "ALF-Corumba", "Outros"))
