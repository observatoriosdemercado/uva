...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 606 toneladas (t). A média histórica (até 2023) para este mês é de 1085 t. Em `r strftime(atual, format = "%B")` de 2022 foram exportadas 603 t e, em `r strftime(atual, format = "%B")` de 2023, foram 889 t, ou seja, houve um aumento de 0,50% em relação a 2022, e queda de -31,8% na comparação com o mesmo período de 2023.

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 1,69 milhões de dólares (US\$). A média histórica (até 2023) para este mês é de aproximadamente US\$ 2,9 milhões. Em `r strftime(atual, format = "%B")` de 2022 foram gerados US\$ 1,72 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2023, um total de US\$ 2,83 milhões.

Os principais destinos no ano foram os Estados Unidos (67,48%), a Holanda (10,65%), a Espanha (8,96%) e o Reino Unido (6,73%). A principal via de envio foi a maritima (99%) e os principais estados de origem da uva foram Pernambuco (74,77%) e Bahia (23,24%). No ano, cerca de 1,9 mil t de uvas foram importadas do Chile e 1,1 mil t da Argentina.
