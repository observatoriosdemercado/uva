...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 17.161 toneladas (t). A média histórica (até 2023) para este mês é de 15.768 t. Em `r strftime(atual, format = "%B")` de 2022 foram exportadas 16.262 t e, em `r strftime(atual, format = "%B")` de 2023, foram 26.574 t, ou seja, houve um aumento de 5,53% em relação a 2022, e queda de -35,42% na comparação com o mesmo período de 2023. A análise dos onze meses de 2024, mostra uma queda de -29,87% no volume exportado de uva pelo Brasil em relação a 2023.

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 40,3 milhões de dólares (US\$). A média histórica (até 2023) para este mês é de aproximadamente US\$ 34,8 milhões. Em `r strftime(atual, format = "%B")` de 2022 foram gerados US\$ 34,2 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2023, um total de US\$ 65,9 milhões.

Os principais destinos no ano foram a Holanda (32,2%), os Estados Unidos (22,35%), o Reino Unido (21,66%) e a Espanha (8,28%). A principal via de envio foi a maritima (91%), com 45,66% do total despachado pela alfândega de Salvador e 44,16% pela de Fortaleza. 

Os principais estados de origem da uva foram Pernambuco (68,6%) e Bahia (28,9%). No ano, cerca de 6,4 mil t de uvas foram importadas do Chile, 1,4 mil t da Argentina, 822 t do Peru, 64 t do Egito e 1 t de Portugal.
...
dados21$URF <- factor(dados21$URF, levels = c("ALF-Salvador", "ALF-Fortaleza", "ALF-Foz", "IRF Natal","Aeroporto de GRU", "ALF-Corumba", "Outros"))
