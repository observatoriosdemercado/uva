...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 761 toneladas (t). A média histórica (até 2023) para este mês é de 2344 t. Em `r strftime(atual, format = "%B")` de 2022 foram exportadas 2447 t e, em `r strftime(atual, format = "%B")` de 2023, foram 3694 t, ou seja, houve uma redução de -68,9% em relação a 2022, e de -79,4% na comparação com o mesmo período de 2023.  A análise do primeiro quadrimestre de 2024 com 2023, mostra uma queda de -37% no volume exportado de uva pelo Brasil.

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 1,82 milhões de dólares (US\$). A média histórica (até 2023) para este mês é de aproximadamente US\$ 5,6 milhões. Em `r strftime(atual, format = "%B")` de 2022 foram gerados US\$ 5,17 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2023, um total de US\$ 9 milhões.

Os principais destinos no ano foram os Estados Unidos (67,59%), a Holanda (11,50%), o Reino Unido (7,56%) e a Espanha (7,22%). A principal via de envio foi a maritima (99%) e os principais estados de origem da uva foram Pernambuco (70,51%) e Bahia (27,49%). No ano, cerca de 3,5 mil t de uvas foram importadas do Chile e 1,4 mil t da Argentina.
...
# 6. Unidade da Federação da uva exportada do Brasil em 2024.
