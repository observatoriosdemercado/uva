...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 460 toneladas (t). A média histórica (até 2023) para este mês é de 3626 t. Em `r strftime(atual, format = "%B")` de 2022 foram exportadas 4049 t e, em `r strftime(atual, format = "%B")` de 2023, foram 7902 t, ou seja, houve uma redução de -88,64% em relação a 2022, e de -94,18% na comparação com o mesmo período de 2023.  A análise dos primeiros cinco meses de 2024, mostra uma queda de -69% no volume exportado de uva pelo Brasil em relação a 2023.

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 1,19 milhões de dólares (US\$). A média histórica (até 2023) para este mês é de aproximadamente US\$ 8,16 milhões. Em `r strftime(atual, format = "%B")` de 2022 foram gerados US\$ 8,45 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2023, um total de US\$ 19,9 milhões.

Os principais destinos no ano foram os Estados Unidos (61%), a Holanda (12,7%), o Reino Unido (10,2%) e a Espanha (8,17%). A principal via de envio foi a maritima (96,4%) e os principais estados de origem da uva foram Pernambuco (67,8%) e Bahia (29,4%). No ano, cerca de 5,4 mil t de uvas foram importadas do Chile e 1,4 mil t da Argentina.
