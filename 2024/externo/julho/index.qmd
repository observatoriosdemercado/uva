...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 538 toneladas (t). A média histórica (até 2023) para este mês é de 408 t. Em `r strftime(atual, format = "%B")` de 2022 foram exportadas 465 t e, em `r strftime(atual, format = "%B")` de 2023, foram 748 t, ou seja, houve um aumento de 15,70% em relação a 2022, e uma redução de -28,07% na comparação com o mesmo período de 2023. A análise dos sete meses de 2024, mostra uma queda de -66,74% no volume exportado de uva pelo Brasil em relação a 2023.

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 1,24 milhões de dólares (US\$). A média histórica (até 2023) para este mês é de aproximadamente US\$ 773 mil. Em `r strftime(atual, format = "%B")` de 2022 foram gerados US\$ 937 mil em receitas e, em `r strftime(atual, format = "%B")` de 2023, um total de US\$ 1,64 milhões.

Os principais destinos no ano foram os Estados Unidos (48,38%), o Reino Unido (16,73%), a Holanda (10,51%) e a Argentina (9%). A principal via de envio foi a maritima (85,6%) e os principais estados de origem da uva foram Pernambuco (61%) e Bahia (33,48%). No ano, cerca de 8,2 mil t de uvas foram importadas do Chile, 1,4 mil t da Argentina e 392 t do Peru. Em julho, também ocorreu a importação de 48 t de uvas do Egito.
