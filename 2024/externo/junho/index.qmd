...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 610 toneladas (t). A média histórica (até 2023) para este mês é de 1.600 t. Em `r strftime(atual, format = "%B")` de 2022 foram exportadas 3.013 t e, em `r strftime(atual, format = "%B")` de 2023, foram 1.801 t, ou seja, houve uma redução de -79,75% em relação a 2022, e de -66,13% na comparação com o mesmo período de 2023.  A análise dos primeiros seis meses de 2024, mostra uma queda de -68,56% no volume exportado de uva pelo Brasil em relação a 2023.

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 1,49 milhões de dólares (US\$). A média histórica (até 2023) para este mês é de aproximadamente US\$ 2,58 milhões. Em `r strftime(atual, format = "%B")` de 2022 foram gerados US\$ 576 mil em receitas e, em `r strftime(atual, format = "%B")` de 2023, um total de US\$ 3,8 milhões.

Os principais destinos no ano foram os Estados Unidos (53,6%), o Reino Unido (15,73%), a Holanda (11,64%) e a Espanha (7,17%). A principal via de envio foi a maritima (92%) e os principais estados de origem da uva foram Pernambuco (62,6%) e Bahia (33,45%). No ano, cerca de 6,3 mil t de uvas foram importadas do Chile, 1,4 mil t da Argentina e 204 t do Peru.
