...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 801 toneladas (t). A média histórica (até 2023) para este mês é de 423 t. Em `r strftime(atual, format = "%B")` de 2022 foram exportadas 721 t e, em `r strftime(atual, format = "%B")` de 2023, foram 191 t, ou seja, houve um aumento de 11,09% em relação a 2022, e de 319% na comparação com o mesmo período de 2023.

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 1,9 milhões de dólares (US\$). A média histórica (até 2023) para este mês é de aproximadamente US\$ 1,1 milhões. Em `r strftime(atual, format = "%B")` de 2022 foram gerados US\$ 2 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2023, um total de US\$ 651 mil.

Os principais destinos no ano foram os Estados Unidos (68,41%), a Holanda (10,28%), a Espanha (7,66%) e o Reino Unido (6,22%). A principal via de envio foi a maritima (98%) e os principais estados de origem da uva foram Pernambuco (72%) e Bahia (24%). No ano, cerca de 486 t de uvas foram importadas do Chile e 397 t da Argentina.
