...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 828 toneladas (t). A média histórica (até 2023) para este mês é de 554 t. Em `r strftime(atual, format = "%B")` de 2022 foram exportadas 843 t e, em `r strftime(atual, format = "%B")` de 2023, foram 751 t, ou seja, houve uma queda de -1,78% em relação a 2022, e um aumento de 10,25% na comparação com o mesmo período de 2023. A análise dos oito meses de 2024, mostra uma queda de -63,41% no volume exportado de uva pelo Brasil em relação a 2023.

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 2,12 milhões de dólares (US\$). A média histórica (até 2023) para este mês é de aproximadamente US\$ 1,08 milhões. Em `r strftime(atual, format = "%B")` de 2022 foram gerados US\$ 1,67 nilhões em receitas e, em `r strftime(atual, format = "%B")` de 2023, um total de US\$ 1,90 milhões.

Os principais destinos no ano foram os Estados Unidos (42,60%), a Argentina (19,23%) e o Reino Unido (14,91%). A principal via de envio foi a maritima (75,4%), com 49% do total despachado pela alfândega de Fortaleza e 25% pela de Salvado. 

Os principais estados de origem da uva foram Pernambuco (58,03%) e Bahia (35,35%). No ano, cerca de 6,4 mil t de uvas foram importadas do Chile, 1,4 mil t da Argentina, 455 t do Peru e 64 t do Egito.
