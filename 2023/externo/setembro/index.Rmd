...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 3.388 toneladas (t). A média histórica (até 2020) para este mês é de 3.787 t. Em `r strftime(atual, format = "%B")` de 2021 foram exportadas 1.993 t e, em `r strftime(atual, format = "%B")` de 2022, foram 2.279 t, ou seja, houve um aumento de 70\% em relação a 2021 e de 48,66\% na comparação com o mesmo período de 2022. Nos nove meses do ano, foram exportadas cerca de 20,8 mil t, um aumento de 33,51\% em relação ao mesmo período de 2022.

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 9,1 milhões de dólares (US\$). A média histórica (até 2020) para este mês é de aproximadamente US\$ 8,1 milhões. Em `r strftime(atual, format = "%B")` de 2021 foram gerados US\$ 4,03 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2022, um total de US\$ 5,26 milhões. Assim, apesar do menor volume, as receitas aumentaram, indicando uma valorização da uva brasileira. 

Os principais destinos no ano foram a Holanda (30\%), os Estados Unidos (27,45\%), o Reino Unido (24,46\%) e a Argentina (9,17\%). A principal via de envio foi a maritima (87,81\%) e os principais estados de origem da uva foram Pernambuco (62,15%) e Bahia (34,02\%). Em `r strftime(atual, format = "%B")`, cerca de 154 t de uvas foram importadas do Peru.

A taxa de câmbio esperada pelo mercado para o final do ano de 2023 é de R$4,94 para 1 dólar.
