...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 26.574 toneladas (t). A média histórica (até 2020) para este mês é de 12.266 t. Em `r strftime(atual, format = "%B")` de 2021 foram exportadas 21.976 t e, em `r strftime(atual, format = "%B")` de 2022, foram 16.262 t, ou seja, houve um aumento de 63,4\% em relação a 2022, e de 20,92\% na comparação com o mesmo período de 2021. Nos onze meses do ano, foram exportadas cerca de 62,3 mil t, um aumento de 44\% em relação ao mesmo período de 2022.

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 60,8 milhões de dólares (US\$). A média histórica (até 2020) para este mês é de aproximadamente US\$ 26,6 milhões. Em `r strftime(atual, format = "%B")` de 2021 foram gerados US\$ 45 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2022, um total de US\$ 34,2 milhões. Assim, as receitas proporcionalmente aumentaram mais do que o volume, indicando a valorização da fruta brasileira. 

Os principais destinos no ano foram a Holanda (35,58\%), os Estados Unidos (26,35\%), o Reino Unido (22,69\%) e a Espanha (4,69\%). A principal via de envio foi a maritima (95,2\%) e os principais estados de origem da uva foram Pernambuco (67,44%) e Bahia (31,03\%). Em `r strftime(atual, format = "%B")`, cerca de 210 t de uvas foram importadas do Peru.

A taxa de câmbio esperada pelo mercado para o final do ano de 2023 é de R\$ 4,96 para 1 dólar.
