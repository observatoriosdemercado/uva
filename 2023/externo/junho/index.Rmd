...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 1.815 toneladas (t). A média histórica (até 2020) para este mês é de 814 t. Em `r strftime(atual, format = "%B")` de 2021 foram exportadas 3.915 t e, em `r strftime(atual, format = "%B")` de 2022, foram 3.013 t, ou seja, houve uma redução de -53,64\% em relação a 2021 e de -39,76\% na comparação com o mesmo período de 2022. 

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 3,8 milhões de dólares (US\$). A média histórica (até 2020) para este mês é de aproximadamente US\$ 1,75 milhões. Em `r strftime(atual, format = "%B")` de 2021 foram gerados US\$ 7,5 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2022, um total de US\$ 576 mil. 

Os principais destinos no ano foram a Holanda (33,39\%), os Estados Unidos (26,85\%), o Reino Unido (25,38\%) e a Argentina (5,67\%). A principal via de envio foi a maritima (91,71\%) e os principais estados de origem da uva foram Pernambuco (60,03%) e Bahia (36,36\%). Em `r strftime(atual, format = "%B")`, cerca de 330 t de uvas foram importadas do Chile e 26 t do Peru.

A taxa de câmbio esperada pelo mercado para o final do ano de 2023 é de R$4,98 para 1 dólar.
