...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 14.983 toneladas (t). A média histórica (até 2020) para este mês é de 15.392 t. Em `r strftime(atual, format = "%B")` de 2021 foram exportadas 19.850 t e, em `r strftime(atual, format = "%B")` de 2022, foram 11.473 t, ou seja, houve um aumento de 31\% em relação a 2022, mas uma queda de -24,52\% na comparação com o mesmo período de 2022. Nos dez meses do ano, foram exportadas cerca de 35,7 mil t, um aumento de 32,27\% em relação ao mesmo período de 2022.

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 35,6 milhões de dólares (US\$). A média histórica (até 2020) para este mês é de aproximadamente US\$ 32,9 milhões. Em `r strftime(atual, format = "%B")` de 2021 foram gerados US\$ 40 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2022, um total de US\$ 25,9 milhões. Assim, as receitas proporcionalmente aumentaram mais do que o volume, indicando a valorização da fruta brasileira. 

Os principais destinos no ano foram os Estados Unidos (30,20\%), a Holanda (29,30\%), o Reino Unido (23,40\%) e a Argentina (6\%). A principal via de envio foi a maritima (92\%) e os principais estados de origem da uva foram Pernambuco (65,1%) e Bahia (32,3\%). Em `r strftime(atual, format = "%B")`, cerca de 206 t de uvas foram importadas do Peru.

A taxa de câmbio esperada pelo mercado para o final do ano de 2023 é de R\$ 5,00 para 1 dólar.
