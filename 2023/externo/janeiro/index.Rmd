...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 1.402 toneladas (t). A média histórica (até 2020) para este mês é de 1.025 t. Em `r strftime(atual, format = "%B")` de 2021 foram exportadas 950 t e, em `r strftime(atual, format = "%B")` de 2022, foram 1.124 t, ou seja, houve um aumento de 47,58\% em relação a 2021 e de 24,73\% na comparação com o mesmo período de 2022. 

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 2,98 milhões de dólares (US\$). A média histórica (até 2020) para este mês é de aproximadamente US\$ 2,35 milhões. Em `r strftime(atual, format = "%B")` de 2021 foram gerados US\$ 2,34 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2022, um total de US\$ 2,85 milhões. 

Os principais destinos no ano foram a Holanda (30,60\%), os Estados Unidos (27,54\%), a Espanha (23,81\%) e o Reino Unido (13,12\%). A principal via de envio foi a maritima (98,63\%) e os principais estados de origem da uva foram Pernambuco (48,70%) e Bahia (47,79\%). Em `r strftime(atual, format = "%B")`, cerca de 106 t de uvas foram importadas do Chile, 87 t da Argentina e 78t do Peru.
