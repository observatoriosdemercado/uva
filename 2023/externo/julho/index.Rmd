...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 748 toneladas (t). A média histórica (até 2020) para este mês é de 249 t. Em `r strftime(atual, format = "%B")` de 2021 foram exportadas 804 t e, em `r strftime(atual, format = "%B")` de 2022, foram 465 t, ou seja, houve uma redução de -6,97\% em relação a 2021 e aumento de 60,86\% na comparação com o mesmo período de 2022. 

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 1,6 milhões de dólares (US\$). A média histórica (até 2020) para este mês é de aproximadamente US\$ 489 mil. Em `r strftime(atual, format = "%B")` de 2021 foram gerados US\$ 1,17 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2022, um total de US\$ 937 mil. 

Os principais destinos no ano foram a Holanda (32,32\%), os Estados Unidos (25,97\%), o Reino Unido (24,71\%) e a Argentina (8,28\%). A principal via de envio foi a maritima (88,87\%) e os principais estados de origem da uva foram Pernambuco (60,46%) e Bahia (35,73\%). Em `r strftime(atual, format = "%B")`, cerca de 88 t de uvas foram importadas do Peru.

A taxa de câmbio esperada pelo mercado para o final do ano de 2023 é de R$4,91 para 1 dólar.
