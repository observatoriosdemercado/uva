...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 7.908 toneladas (t). A média histórica (até 2020) para este mês é de 1.988 t. Em `r strftime(atual, format = "%B")` de 2021 foram exportadas 7.117 t e, em `r strftime(atual, format = "%B")` de 2022, foram 4.049 t, ou seja, houve um aumento de 11,11\% em relação a 2021 e de 95,31\% na comparação com o mesmo período de 2022. 

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 16,9 milhões de dólares (US\$). A média histórica (até 2020) para este mês é de aproximadamente US\$ 4,3 milhões. Em `r strftime(atual, format = "%B")` de 2021 foram gerados US\$ 15,3 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2022, um total de US\$ 8,5 milhões. 

Os principais destinos no ano foram a Holanda (33,17\%), os Estados Unidos (30,3\%), o Reino Unido (24,17\%) e a Argentina (3,85\%). A principal via de envio foi a maritima (94,27\%) e os principais estados de origem da uva foram Pernambuco (60,94%) e Bahia (36,27\%). Em `r strftime(atual, format = "%B")`, cerca de 1,1 mil t de uvas foram importadas do Chile.

A taxa de câmbio esperada pelo mercado para o final do ano de 2023 é de R$5,11 para 1 dólar.
