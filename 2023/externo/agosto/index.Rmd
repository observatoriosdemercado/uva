...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 751 toneladas (t). A média histórica (até 2020) para este mês é de 391 t. Em `r strftime(atual, format = "%B")` de 2021 foram exportadas 879 t e, em `r strftime(atual, format = "%B")` de 2022, foram 843 t, ou seja, houve uma redução de -14,56\% em relação a 2021 e de -10,91\% na comparação com o mesmo período de 2022. Nos oito meses do ano, foram exportadas cerca de 17,4 mil t, um aumento de 30,91\% em relação ao mesmo período de 2022.

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 1,8 milhões de dólares (US\$). A média histórica (até 2020) para este mês é de aproximadamente US\$ 734 mil. Em `r strftime(atual, format = "%B")` de 2021 foram gerados US\$ 1,43 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2022, um total de US\$ 1,67 milhões. Assim, apesar do menor volume, as receitas aumentaram, indicam uma valorização da uva brasileira. 

Os principais destinos no ano foram a Holanda (31\%), os Estados Unidos (25,58\%), o Reino Unido (23,92\%) e a Argentina (10,54\%). A principal via de envio foi a maritima (86,16\%) e os principais estados de origem da uva foram Pernambuco (60,06%) e Bahia (35,41\%). Em `r strftime(atual, format = "%B")`, cerca de 132 t de uvas foram importadas do Peru.

A taxa de câmbio esperada pelo mercado para o final do ano de 2023 é de R$4,95 para 1 dólar.
