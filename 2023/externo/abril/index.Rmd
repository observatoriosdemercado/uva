...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 889 toneladas (t). A média histórica (até 2020) para este mês é de 3.694 t. Em `r strftime(atual, format = "%B")` de 2021 foram exportadas 4.807 t e, em `r strftime(atual, format = "%B")` de 2022, foram 2.447 t, ou seja, houve uma redução de -23,15\% em relação a 2021 e um aumento de 50,96\% na comparação com o mesmo período de 2022. 

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 7,8 milhões de dólares (US\$). A média histórica (até 2020) para este mês é de aproximadamente US\$ 3,7 milhões. Em `r strftime(atual, format = "%B")` de 2021 foram gerados US\$ 12 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2022, um total de US\$ 5,17 milhões. 

Os principais destinos no ano foram os Estados Unidos (44,46\%), a Holanda (24,79\%), o Reino Unido (16,53\%) e a Espanha (5,61\%). A principal via de envio foi a maritima (96,26\%) e os principais estados de origem da uva foram Pernambuco (59,63%) e Bahia (36,94\%). Em `r strftime(atual, format = "%B")`, cerca de 1,3 mil t de uvas foram importadas do Chile, 18 t da Argentina.

A taxa de câmbio esperada pelo mercado para o final do ano de 2023 é de R$5,18 para 1 dólar.
