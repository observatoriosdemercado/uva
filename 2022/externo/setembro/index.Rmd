...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 2.279 toneladas (t). A média histórica (até 2019, antes da pandemia) para este mês é de 3.888 t. Em `r strftime(atual, format = "%B")` de 2020 foram exportadas 3.382 t e, em `r strftime(atual, format = "%B")` de 2021, foram 1.993 t, ou seja, houve uma redução de 32,61% em relação a 2020 e um aumento de 14,35\% na comparação com o mesmo período de 2021. Nos nove meses se tem uma redução de -38,85\% no volume exportado em comparação com o mesmo período do ano passado. 

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 5 milhões de dólares (US\$). A média histórica (até 2019, antes da pandemia) para este mês é de aproximadamente US\$ 8,3 milhões. Em `r strftime(atual, format = "%B")` de 2020 foram gerados US\$ 7,4 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2021, um total de US\$ 4,1 milhões.

Os principais destinos nos nove meses do ano foram a Holanda (26,70\%), o Reino Unido (26,16\%) e os Estados Unidos (19,56\%). A principal via de envio foi a maritima (80\%) e os principais estados de origem da uva foram Pernambuco (60%) e Bahia (34,62\%). Em `r strftime(atual, format = "%B")`, cerca de 21,7 t foram importadas do Peru. Também houve importação de uva do Egito.
