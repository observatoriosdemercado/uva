...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 465 toneladas (t). A média histórica (até 2019, antes da pandemia) para este mês é de 207 t. Em `r strftime(atual, format = "%B")` de 2020 foram exportadas 415 t e, em `r strftime(atual, format = "%B")` de 2021, foram 804 t, ou seja, houve um aumento de 12,05% em relação a 2020 e uma queda de -42,16\% na comparação com o mesmo período de 2021. Nos sete primeiros meses se tem uma redução de -44,90\% no volume exportado em comparação com o mesmo período do ano passado. 

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 955 mil dólares (US\$). A média histórica (até 2019, antes da pandemia) para este mês é de US\$ 422,3 mil. Em `r strftime(atual, format = "%B")` de 2020 foram gerados US\$ 755,7 mil em receitas e, em `r strftime(atual, format = "%B")` de 2021, um total de US\$ 1,16 milhões.

Os principais destinos nos primeiros sete meses do ano foram o Reino Unido (30,71\%), a Holanda (29,18\%) e os Estados Unidos (20,94\%). A principal via de envio foi a maritima (90,06\%) e os principais estados de origem da uva foram Pernambuco (61,06%) e Bahia (35,56\%). Em `r strftime(atual, format = "%B")`, cerca de 310 t de uva foram importadas do Chile, sendo que não ocorreram importações em `r strftime(atual, format = "%B")` de 2021. 
