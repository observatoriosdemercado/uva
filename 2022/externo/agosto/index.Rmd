...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 843 toneladas (t). A média histórica (até 2019, antes da pandemia) para este mês é de 363 t. Em `r strftime(atual, format = "%B")` de 2020 foram exportadas 505 t e, em `r strftime(atual, format = "%B")` de 2021, foram 879 t, ou seja, houve um aumento de 66,93% em relação a 2020 e uma queda de -4,09\% na comparação com o mesmo período de 2021. Nos oito meses se tem uma redução de -43,38\% no volume exportado em comparação com o mesmo período do ano passado. 

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 1,6 milhões de dólares (US\$). A média histórica (até 2019, antes da pandemia) para este mês é de US\$ 686,4 mil. Em `r strftime(atual, format = "%B")` de 2020 foram gerados US\$ 926,2 mil em receitas e, em `r strftime(atual, format = "%B")` de 2021, um total de US\$ 1,43 milhões.

Os principais destinos nos primeiros sete meses do ano foram o Reino Unido (28,96\%), a Holanda (27,33\%) e os Estados Unidos (19,93\%). A principal via de envio foi a maritima (85,11\%) e os principais estados de origem da uva foram Pernambuco (60,69%) e Bahia (35,17\%). Em `r strftime(atual, format = "%B")`, cerca de 43,5 t de uva foram importadas do Chile e 56,1 t do Peru.
