...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 4061 toneladas (t). A média histórica (até 2019, antes da pandemia) para este mês é de 1632 t. Em `r strftime(atual, format = "%B")` de 2020 foram exportadas 3410 t e, em `r strftime(atual, format = "%B")` de 2021, foram 7117 t, ou seja, houve um aumento de 19,09% em relação a 2020 e uma queda de -42,94% na comparação com o mesmo período de 2021. Nos cinco primeiros meses se tem uma redução de -49,77% no volume exportado em comparação com o mesmo período do ano passado. 

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 7,71 milhões de dólares (US\$). A média histórica (até 2019, antes da pandemia) para este mês é de US\$ 3,68 milhões. Em `r strftime(atual, format = "%B")` de 2020 foram gerados US\$ 6,93 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2021, um total de US\$ 15,3 milhões.

Os principais destinos primeiro trimestre do ano foram a Holanda (30,90\%), os Estados Unidos (28,72\%) e o Reino Unido (27,24%). A principal via de envio foi a maritima (96,46%) e os principais estados de origem da uva foram Pernambuco (64,80%) e Bahia (33,46%). Em `r strftime(atual, format = "%B")`, cerca de 1,7 mil t de uva foram importadas do Chile, volume 188\% superior ao que foi importado pelo Brasil em `r strftime(atual, format = "%B")` de 2021. 
