...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 16.274 toneladas (t). A média histórica (até 2019, antes da pandemia) para este mês é de 11.604 t. Em `r strftime(atual, format = "%B")` de 2020 foram exportadas 14.912 t e, em `r strftime(atual, format = "%B")` de 2021, foram 21.976 t, ou seja, houve um aumento de 9,13\% em relação a 2020 e uma queda de -25,94\% na comparação com o mesmo período de 2021. Nos onze meses se tem uma redução de -35,62\% no volume exportado em comparação com o mesmo período do ano passado. 

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 30,4 milhões de dólares (US\$). A média histórica (até 2019, antes da pandemia) para este mês é de aproximadamente US\$ 24,9 milhões. Em `r strftime(atual, format = "%B")` de 2020 foram gerados US\$ 33,7 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2021, um total de US\$ 45,2 milhões.

Os principais destinos nos onze meses do ano foram a Holanda (34,24\%), o Reino Unido (26,70\%) e os Estados Unidos (17,06\%). A principal via de envio foi a maritima (90,25\%) e os principais estados de origem da uva foram Pernambuco (67,26%) e Bahia (30,12\%). Em `r strftime(atual, format = "%B")`, cerca de 86 t de uvas foram importadas do Peru.
