...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 9.284 toneladas (t). A média histórica (até 2019, antes da pandemia) para este mês é de 2.960 t. Em `r strftime(atual, format = "%B")` de 2020 foram exportadas 5.523 t e, em `r strftime(atual, format = "%B")` de 2021, foram 9.363 t, ou seja, houve um aumento de 68,1\% em relação a 2020 e uma queda de -0,84\% na comparação com o mesmo período de 2021. No ano se tem uma redução de -31,39\% no volume exportado em comparação com o ano passado. 

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 18,6 milhões de dólares (US\$). A média histórica (até 2019, antes da pandemia) para este mês é de aproximadamente US\$ 6,65 milhões. Em `r strftime(atual, format = "%B")` de 2020 foram gerados US\$ 11,8 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2021, um total de US\$ 17,6 milhões. No ano se tem uma redução de -32,33\% nas receitas obtidas em comparação com o ano passado. 

Os principais destinos no ano foram a Holanda (35,91\%), o Reino Unido (25,10\%) e os Estados Unidos (18,33\%). A principal via de envio foi a maritima (91,64\%) e os principais estados de origem da uva foram Pernambuco (67,93%) e Bahia (29,73\%). Em `r strftime(atual, format = "%B")`, cerca de 121 t de uvas foram importadas do Peru.

A taxa de câmbio esperada pelo mercado para o final do ano de 2023 é de R$5,26 para 1 dólar.
...
g10 <- ggplot(data=dados3) +  #estetica vai valer para todos os geom's
  ...
  labs(y= "% do Volume Total Exportados", x= "Países", title='',
       caption = "")+
  scale_y_continuous(limits=c(0, 36), n.breaks = 10, expand = expansion(add=c(0,0.5)))+
   theme_classic()+ #Definindo tema
