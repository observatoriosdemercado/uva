...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 11.473 toneladas (t). A média histórica (até 2019, antes da pandemia) para este mês é de 15.702 t. Em `r strftime(atual, format = "%B")` de 2020 foram exportadas 14.154 t e, em `r strftime(atual, format = "%B")` de 2021, foram 19.850 t, ou seja, houve uma redução de 18,94\% em relação a 2020 e de 42,20\% na comparação com o mesmo período de 2021. Nos dez meses se tem uma redução de -40,32\% no volume exportado em comparação com o mesmo período do ano passado. 

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 23 milhões de dólares (US\$). A média histórica (até 2019, antes da pandemia) para este mês é de aproximadamente US\$ 33,4 milhões. Em `r strftime(atual, format = "%B")` de 2020 foram gerados US\$ 30,7 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2021, um total de US\$ 40 milhões.

Os principais destinos nos dez meses do ano foram a Holanda (30,13\%), o Reino Unido (27,96\%) e os Estados Unidos (19,06\%). A principal via de envio foi a maritima (86,5\%) e os principais estados de origem da uva foram Pernambuco (63,79%) e Bahia (32,60\%). Em `r strftime(atual, format = "%B")`, cerca de 130 t foram importadas do Peru.
