...
O objetivo desta análise foi verificar o comportamento das exportações de uva do Brasil em `r strftime(atual, format = "%B de %Y")`. Os dados utilizados são mensais e disponibilizados pelo **Comexstat (MDIC)** para o período de *Janeiro de 2016 a `r strftime(atual, format = "%B de %Y")`*;

Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 3013 toneladas (t). A média histórica (até 2019, antes da pandemia) para este mês é de 676 t. Em `r strftime(atual, format = "%B")` de 2020 foram exportadas 1366 t e, em `r strftime(atual, format = "%B")` de 2021, foram 3915 t, ou seja, houve um aumento de 120,6% em relação a 2020 e uma queda de -23,05% na comparação com o mesmo período de 2021. Nos seis primeiros meses se tem uma redução de -45,01% no volume exportado em comparação com o mesmo período do ano passado. 

Em termos dos valores da exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de 5,7 milhões de dólares (US\$). A média histórica (até 2019, antes da pandemia) para este mês é de US\$ 1,51 milhões. Em `r strftime(atual, format = "%B")` de 2020 foram gerados US\$ 2,72 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2021, um total de US\$ 7,5 milhões.

Os principais destinos primeiro trimestre do ano foram o Reino Unido (31,36\%), a Holanda (30,32\%) e os Estados Unidos (21,75\%). A principal via de envio foi a maritima (92,49\%) e os principais estados de origem da uva foram Pernambuco (62,38%) e Bahia (35,19\%). Em `r strftime(atual, format = "%B")`, cerca de 555 t de uva foram importadas do Chile, volume 18,8\% superior ao que foi importado pelo Brasil em `r strftime(atual, format = "%B")` de 2021. 
...
Obs: Dados de 2022 até o mês de `r strftime(atual, format = "%B")`.
...
## Valor das Exportações de uva do Brasil: Janeiro a `r strftime(atual, format = "%B")` de 2022 em comparação com 2016-2021 {.tabset .tabset-fade}
...
## Volume das Exportações de uva do Brasil: Janeiro a `r strftime(atual, format = "%B")` de 2022 em comparação com 2016-2021 {.tabset .tabset-fade}
...
Obs: Dados disponibilizados de projeções realizadas até o final de abril de 2022
