...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 912 toneladas (t). A média histórica (até 2025) para este mês é de 451 t. Em `r strftime(atual, format = "%B")` de 2024 foram exportadas 801 t e, em `r strftime(atual, format = "%B")` de 2025, foram 322 t, ou seja, houve um aumento de 13,86% em relação a 2024 e de 183,23% na comparação com o mesmo período de 2025. No ano, contudo, o volume de 1.704 t é menor do que nos dois últimos anos.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de US\$ 2,28 milhões de dólares, abaixo da média histórica de US\$ 1,24 milhões. Em `r strftime(atual, format = "%B")` de 2024 foram gerados US\$ 2,4 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2025, um total de US\$ 901 mil. Assim, pode-se considerar que os preços em `r strftime(atual, format = "%B")` foram mais baixos do que a média histórica e os dois anos anteriores.

Os principais destinos foram a Holanda (50,1%), o Reino Unido (15,78%) e o Canadá (12,39%). Começam a aparecer os embarques para a China, depois da abertura do mercado. Em janeiro foram 1,5 t e em fevereiro, 3,5 t embarcadas de forma área. A principal via de envio foi a maritima (96,36%), com 89,7% do total despachado pela alfândega de Fortaleza. Os principais estados de origem da uva foram Pernambuco (65,95%) e Bahia (31,64%). Em `r strftime(atual, format = "%B de %Y")` foram importadas 819 t de uvas do Chile, 292 t da Argentina e 54 t do Peru.
...
dados21$URF <- factor(dados21$URF, levels = c("ALF-Fortaleza", "ALF-Salvador", "ALF-Dionísio Cerqueira", "ALF-Foz do Iguaçu", "Aeroporto de GRU", "Outros"))
