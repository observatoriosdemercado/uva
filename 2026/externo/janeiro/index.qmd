...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 792 toneladas (t). A média histórica (até 2025) para este mês é de 1.242 t. Em `r strftime(atual, format = "%B")` de 2024 foram exportadas 1.754 t e, em `r strftime(atual, format = "%B")` de 2025, foram 2.062 t, ou seja, houve uma redução de -54,84% em relação a 2024 e uma queda -61,59% na comparação com o mesmo período de 2025.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de US\$ 2,2 milhões de dólares, abaixo da média histórica de US\$ 3 milhões. Em `r strftime(atual, format = "%B")` de 2024 foram gerados US\$ 5,36 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2025, um total de US\$ 5,2 milhões.

Os principais destinos foram a Holanda (46,8%), o Canadá (15%), o Reino Unido (13,3%) e os Estados Unidos (9,84%). A principal via de envio foi a maritima (96,76%), com 92% do total despachado pela alfândega de Fortaleza. Os principais estados de origem da uva foram Pernambuco (53,38%) e Bahia (44,7%). Em `r strftime(atual, format = "%B de %Y")` foram importadas 206 t de uvas do Chile, 155 t da Argentina e 491 t do Peru.

A taxa de câmbio esperada pelo mercado para o final do ano de 2026 é de R$5,48 para 1 dólar.
...
dados21$URF <- factor(dados21$URF, levels = c("ALF-Fortaleza", "ALF-Salvador", "ALF-Foz do Iguaçu", "Aeroporto de GRU", "Porto de Santos", "Outros"))
