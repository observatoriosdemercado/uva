...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 580 toneladas (t). A média histórica (até 2025) para este mês é de 977 t. Em `r strftime(atual, format = "%B")` de 2024 foram exportadas 606 t e, em `r strftime(atual, format = "%B")` de 2025, foram 480 t, ou seja, houve uma redução de -4,29% em relação a 2024 e um aumento de 20,83% na comparação com o mesmo período de 2025. No ano, o volume de 2.285 t é menor do que nos dois últimos anos.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de US\$ 1,46 milhões de dólares, abaixo da média histórica de US\$ 2,69 milhões. Em `r strftime(atual, format = "%B")` de 2024 foram gerados US\$ 2,1 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2025, um total de US\$ 1,37 mil. Assim, pode-se considerar que os preços em `r strftime(atual, format = "%Y")` foram mais altos do que o mesmo mês nos dois anos anteriores.

Os principais destinos foram a Holanda (58,76%), o Reino Unido (12,66%) e o Canadá (9,67%). A principal via de envio foi a maritima (96,09%), com 85,88% do total despachado pela alfândega de Fortaleza. Os principais estados de origem da uva foram Pernambuco (65,72%) e Bahia (30,34%). Em `r strftime(atual, format = "%B de %Y")` foram importadas 2.093 t de uvas do Chile, 320 t da Argentina e 14.2 t do Peru.

A taxa de câmbio esperada pelo mercado para o final do ano de 2026 é de R$5,40 para 1 dólar.
