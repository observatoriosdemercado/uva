...
Em relação aos volumes, em `r strftime(atual, format = "%B de %Y")` foram de 1.253 toneladas (t). A média histórica (até 2025) para este mês é de 2.090 t. Em `r strftime(atual, format = "%B")` de 2024 foram exportadas 761 t e, em `r strftime(atual, format = "%B")` de 2025, foram 1.383 t, ou seja, houve uma redução de -9,39% em relação a 2025 e um aumento de 64,65% na comparação com o mesmo período de 2024. No ano, o volume de 3.537 t é menor do que nos dois últimos anos.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B de %Y")` foram cerca de US\$ 3,3 milhões de dólares, abaixo da média histórica de US\$ 5,1 milhões. Em `r strftime(atual, format = "%B")` de 2024 foram gerados US\$ 2 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2025, um total de US\$ 3,83 milhões. 

Os principais destinos foram a Holanda (49,74%), o Reino Unido (17,16%) e os Estados Unidos (10,68%). A principal via de envio foi a maritima (91,79%), com 70,82% do total despachado pela alfândega de Fortaleza. Os principais estados de origem da uva foram Pernambuco (66,1%) e Bahia (29,24%). Em `r strftime(atual, format = "%B de %Y")` foram importadas 1.663 t de uvas do Chile e 41.9 t da Argentina.
